...
export class PaymentService {
  private service: BaseService<PaymentEntity>;

  constructor(
    private readonly dataSource: DataSource,
    private readonly httpExceptionService: HttpExceptionService,
  ) {
    this.service = new BaseService<PaymentEntity>(this.dataSource.getRepository(PaymentEntity));
  }
  ...
  ): Promise<PaginationResponseDto<PaymentEntity>> {
  ...
  async create(object: PaymentCreateRequestDto) {
    try {
      const entity = new PaymentEntity();
      entity.method = object.method;
  ...
  async updateById(id: string, object: PaymentUpdateRequestDto) {
    try {
