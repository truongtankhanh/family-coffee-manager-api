...
export class ProductCategoryService {
  private service: BaseService<ProductCategoryEntity>;

  constructor(
    private readonly dataSource: DataSource,
    private readonly httpExceptionService: HttpExceptionService,
  ) {
    this.service = new BaseService<ProductCategoryEntity>(
      this.dataSource.getRepository(ProductCategoryEntity),
    );
  }
  ...
  ): Promise<PaginationResponseDto<ProductCategoryEntity>> {
  ...
  async create(object: ProductCategoryCreateRequestDto) {
    try {
  ...
  async updateById(id: string, object: ProductCategoryUpdateRequestDto) {
    try {
